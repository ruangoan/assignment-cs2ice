...
@Component({
  ...
  templateUrl: './geelong.component.html',
  styleUrls: ['./geelong.component.css']
})
export class GeelongComponent implements OnInit {
  ...
  getGames(): void {
    ...
    .subscribe(temp => {this.games = temp
      .filter(
      (team: any) =>
        (team.complete == 100 && team.ateam ) =='Geelong' ||
        (team.complete == 100 && team.hteam ) =='Geelong'
    );})
  ...
  getTips(): void {
    ...
    .filter(
    (tip: any) =>
      (tip.round>'11' &&tip.ateam ) =='Geelong' ||
      (tip.round>'11' &&tip.hteam ) =='Geelong'
  );}); 
  }
}
